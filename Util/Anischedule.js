...
// Hard-Requires
const schedule = require('require-text')('../src/assets/gql/Schedule.graphql', require); 
const watchlist = require('../src/Models/GuildWatching')
const anilist = require(this.utils.AniListquery)
const consoleUtil = require('../Util/betterConsole')
const constants = require('./constants')

module.exports = class Anischedule{ 
    constructor(client){
        /**
         * The client that represents the scheduler
        ...
        /**
         * queuedNotifications for the current running isntant
         * @type {array}
         */
        this.queuedNotifications = []; 
        this.info = {
            /**
             * Media formats for the data that has been retrieved
             * @type {array}
             */
    ...
    fetch(query, variables){
        return this.utils.AniListquery(query, variables)
    }; 
    getAllWatched(){
    ...
    getAnnouncementEmbed(entry, date){
        ...
        .setColor(entry.media.coverImage.color || '#89cff0')
        .setThumbnail(entry.media.coverImage.large)
        .setAuthor('Popcorn Time!')
        .setTimestamp(date)
        .setDescription([
      ...
      async makeAnnouncement(entry, date){
        ...
        for (const g of list){
          ...
          await  msg.channel.createMessage(embed.create) 
          .then((msg) => {
