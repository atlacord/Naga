...
class TeamAvatar extends Listener {
    ...
    execute(member, oldMember, guildConfig) { // eslint-disable-line
        if(!oldMember.roles.find(r => r.name === 'Team Avatar') && member.roles.find(r => r.name === 'Team Avatar')) {
          ...
          return  server.channel.find(c => c.id === '826851222459514923').then(channel.createMessage(`**A new member joins the fold! A big thanks to ${member.mention} for boosting the server! Please make sure you read the pins for info on how to get the TA colour role and more!`))
        }
        if(oldMember.roles.find(r => r.name === 'Team Avatar') && oldMember.roles.find(r => r.name === 'TA Colour') && !member.roles.find(r => r.name === 'Team Avatar')) {
            ...
            return  server.channel.find(c => c.id === '826851222459514923').then(channel.createMessage(`**A member of TA has left us, thanks to ${member.mention} for their contributions!`))
        }
