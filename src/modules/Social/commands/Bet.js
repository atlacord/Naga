    ...
    async execute({ msg, args }) {
        ...
        profile.findById(msg.author.id, (err, doc) => {
            ...
            if (err) {
                ...
                .catch(() => this.sendError(msg.channel, `The betting machine just broke! You lost **${this.utils.commatize(amount)}** coins from your previous bet.\nThis doesn't usually happen. Please contact TwoDog or soda if you receive this message.`))
            }).catch((err) => this.sendError(msg.channel, `Unable to save the document to the database: ${err}`));
            }
        })
    }
}
