...
abstract class Command extends Base {
	...
	public _execute(e: CommandData): Promise<any> {
		...
		if ((this.expectedArgs && args.length < this.expectedArgs) || (args && args[0] === 'help')) {
			if (this.permissions === 'admin' && (!e.isAdmin || !e.isDev)) {
				return Promise.resolve();
			}
			return this.help(message, guildConfig);
		}

		if (!e.isAdmin && !e.isDev) {
	...
	protected log(message: eris.Message, command: string, writeLog?: boolean) {
		if (writeLog) {
			...
			this.logger.info(`[Comamand] Server: ${(<eris.GuildChannel>message.channel).guild.id}, Channel: ${message.channel.id}, User: ${message.author.username} (${message.author.id}) ${message.content}`);
		}
...
export { Command, CommandInterface };
